...
app.get('/', function(req, res) {
  // 查询所有数据内容
  query('select * from moive_data order by intime', [], function(error, results, fields) {
    if (error) {
      console.log(error);
    } else {
      res.render('index', {
...
app.post('/admin/add', (req, res) => {
  let [_title, _doctor, _country, _year, _poster, _language, _flash, _summary] = [req.body.title, req.body.doctor, req.body.country, req.body.year, req.body.poster, req.body.language, req.body.flash, req.body.summary];

  query(`select * from moive_data where title = '${_title}'`, function(error, results, fields) {
    if (error) {
      console.log(error);
    } else if (!results.length) {
      // 注意添加char test类型的字符串时 需要加''哦
      query(`insert into moive_data(title, doctor,country,year,poster,language,flash,summary) values ('${_title}','${_doctor}','${_country}',${_year},'${_poster}','${_language}','${_flash}','${_summary}')`, [], function(error, results, fields) {
        if (error) {
          console.log(error);
        } else {
